...
namespace FedAllChampionsUtility
{
	class Gangplank : Champion
	{
		...
		private void CastRKS()
		{
			...
			if (!R.IsReady( ))
				return;
			foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsValidTarget() && hero.Health <= (ObjectManager.Player.GetSpellDamage( hero,SpellSlot.R) / 2)))
			{
		...
		private void CastE()
		{
			if(!E.IsReady())
				return;
			if(LXOrbwalker.Mode.LaneClear == LXOrbwalker.CurrentMode)
			{
			...
			if(LXOrbwalker.Mode.Combo != LXOrbwalker.CurrentMode)
				return;
