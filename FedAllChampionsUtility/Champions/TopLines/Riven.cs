...
namespace FedAllChampionsUtility
{
	...
	{        
		...
		{
            Program.Menu.AddSubMenu(new Menu("Combo Settings", "Combo"));
            Program.Menu.AddSubMenu(new Menu("KS Settings", "KS"));
            Program.Menu.AddSubMenu(new Menu("Misc", "Misc"));
            Program.Menu.AddSubMenu(new Menu("Draw Settings", "Draw"));
            Program.Menu.SubMenu("Combo").AddItem(new MenuItem("UseUlti", "Use Ultimate in Combo").SetValue(true));
            Program.Menu.SubMenu("Combo").AddItem(new MenuItem("UseQGapClose", "Use Q to gapclose").SetValue(true));
            Program.Menu.SubMenu("KS").AddItem(new MenuItem("KillStealRActivate", "Activate ulti for KS").SetValue(false));
            Program.Menu.SubMenu("KS").AddItem(new MenuItem("KillStealR", "KS with R2").SetValue(true));
            Program.Menu.SubMenu("KS").AddSubMenu(new Menu("Don't Use R For KS", "NoRKS"));
            foreach (Obj_AI_Hero enemy in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.Team != Player.Team))
            {
                Program.Menu.SubMenu("KS")
                    .SubMenu("NoRKS")
                    .AddItem(new MenuItem(enemy.ChampionName, enemy.ChampionName).SetValue(false));
            }
            Program.Menu.SubMenu("KS").AddItem(new MenuItem("KillStealQ", "KS with Q").SetValue(true));
            Program.Menu.SubMenu("KS").AddItem(new MenuItem("KillStealW", "KS with W").SetValue(true));
            Program.Menu.SubMenu("KS").AddItem(new MenuItem("KillStealT", "KS with Tiamat").SetValue(true));
            Program.Menu.SubMenu("Misc")
                .AddItem(new MenuItem("Flee", "Flee Mode").SetValue(new KeyBind(72, KeyBindType.Press)));
            Program.Menu.SubMenu("Misc").AddSubMenu(new Menu("Auto Stun", "AutoStun"));
            foreach (Obj_AI_Hero enemy in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.Team != Player.Team))
            {
                Program.Menu.SubMenu("Misc")
                    .SubMenu("AutoStun")
            ...
            Program.Menu.SubMenu("Misc").AddItem(new MenuItem("AutoW", "Auto W Enemies in Range").SetValue(false));
            ...
            Program.Menu.SubMenu("Misc").AddItem(new MenuItem("DCFix", "Try Disconnect Fix").SetValue(false));
            Program.Menu.SubMenu("Draw").AddItem(new MenuItem("DrawRanges", "Draw engage range").SetValue(new Circle(false, Color.FromArgb(150, Color.Green))));            

            if (IsSR)
            {
                Program.Menu.SubMenu("Draw").AddItem(new MenuItem("DrawJumps", "Draw Jump spots (always)").SetValue(false));
                Program.Menu.SubMenu("Draw")
                ...
                Program.Menu.AddItem(new MenuItem("WallJump", "Wall Jump").SetValue(new KeyBind(71, KeyBindType.Press)));
        ...
        private static void OnDraw(EventArgs args)
        {
            if (Program.Menu.SubMenu("Draw").Item("DrawRanges").GetValue<Circle>().Active)
            {
                Utility.DrawCircle(Player.Position,
                    (Program.Menu.SubMenu("Combo").Item("UseQGapClose").GetValue<bool>() ? _q.Range + _e.Range : _e.Range),
                    Program.Menu.SubMenu("Draw").Item("DrawRanges").GetValue<Circle>().Color);
            }
            if (IsSR &&
                (Program.Menu.SubMenu("Draw").Item("DrawJumps").GetValue<bool>() ||
                 Program.Menu.SubMenu("Draw").Item("DrawJumps2").GetValue<KeyBind>().Active))
            {
                foreach (WallHopPosition pos in jumpPositions)
                {
                    if (Player.Distance(pos.pA) <=
                        Program.Menu.SubMenu("Draw").Item("DrawJumpsRange").GetValue<Slider>().Value ||
        ...
        public static void OnEnemyGapCloser(ActiveGapcloser gapcloser)
        {
            if (_w.IsReady() && gapcloser.Sender.IsValidTarget(_w.Range) &&
                Program.Menu.SubMenu("Misc").Item("AntiGapcloser").GetValue<bool>())
                _w.Cast();
        }
        ...
        public static void OnPossibleToInterrupt(Obj_AI_Base unit, InterruptableSpell spell)
        {
            if (_w.IsReady() && unit.IsValidTarget(_w.Range) &&
                Program.Menu.SubMenu("Misc").Item("Interrupt").GetValue<bool>())
                _w.Cast();
        }

        private static void Wallhopper_OnGameUpdate(EventArgs args)
        {
            if (!busy && Program.Menu.Item("WallJump").GetValue<KeyBind>().Active && qCount == 2)
            {
        ...
        private static void AutoStun()
        {
            foreach (Obj_AI_Hero enemy in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.Team != Player.Team))
            {
                if (_w.IsReady() && enemy.IsValidTarget(_w.Range) &&
                    Program.Menu.SubMenu("Misc").SubMenu("AutoStun").Item("Stun" + enemy.ChampionName).GetValue<bool>())
                {
                    _w.Cast();
                }
            }
        ...
        public static void Combo(Obj_AI_Base target)
        {
            ...
            if (_r.IsReady() && !ultiReady && noRComboDmg < target.Health &&
                Program.Menu.SubMenu("Combo").Item("UseUlti").GetValue<bool>() && currentTarget is Obj_AI_Hero)
            {
                _r.Cast();
            }
        ...
        private static void GapClose(Obj_AI_Base target)
        {
            ...
            bool useQ = _q.IsReady() && qCount < 2 && Program.Menu.SubMenu("Combo").Item("UseQGapClose").GetValue<bool>();
            ...
            if (useQ && qCount < 2 && _q.IsReady() && qRange > distance && !_e.IsReady())
            {
                double noRComboDmg = DamageCalcNoR(target);
                if (_r.IsReady() && !ultiReady && noRComboDmg < target.Health &&
                    Program.Menu.SubMenu("Combo").Item("UseUlti").GetValue<bool>())
                {
                    _r.Cast();
                }
        ...
        private static void KillSecure()
        {
            foreach (Obj_AI_Hero hero in ObjectManager.Get<Obj_AI_Hero>())
            {
                ...
                {
                    if (ultiReady && Program.Menu.SubMenu("KS").Item("KillStealR").GetValue<bool>() &&
                        hero.IsValidTarget(_r.Range - 30) && GetRDamage(hero) - 20 >= hero.Health &&
                        !Program.Menu.SubMenu("KS").SubMenu("NoRKS").Item(hero.ChampionName).GetValue<bool>())
                    {
                        _r.Cast(hero, aoe: true);
                        IsKSing = false;
                    }
                    else if (Program.Menu.SubMenu("KS").Item("KillStealQ").GetValue<bool>() && _q.IsReady() &&
                             hero.IsValidTarget(_q.Range) && GetQDamage(hero) - 10 >= hero.Health)
                    {
                        _q.Cast(hero.ServerPosition, true);
                    }
                    else if (Program.Menu.SubMenu("KS").Item("KillStealW").GetValue<bool>() && _w.IsReady() &&
                             hero.IsValidTarget(_w.Range) && GetWDamage(hero) - 10 >= hero.Health)
                    {
                        _w.Cast();
                    }
                    else if (Program.Menu.SubMenu("KS").Item("KillStealT").GetValue<bool>() &&
